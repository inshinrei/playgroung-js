...
interface CardTypeInfoPartial extends CardBase {
  id: CardTypeID
  initialInDeckCount: number
  isPrimeTown?: boolean
}

export interface CardTypeInfo extends CardTypeInfoPartial {
...
function calcMaxOrientation(card: CardTypeInfoPartial): number {
  if (
...
export const cards: CardTypeInfo[] = cardsPartial.map(card => ({
  ...card,
  orientationsCount: calcMaxOrientation(card),
